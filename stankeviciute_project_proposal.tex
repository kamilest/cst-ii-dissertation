...
% [Tiago] it's a regression task, so you'll not have accuracy and AUC. Check r^2, pearson correlation (the typical ones in papers), and the ones suggested by Alex (I forgot exactly what he said)
The aim of this project will be to adapt this approach to a regression task on the UK Biobank dataset, predicting the subject's age based on neuroimaging data, and comparing it to another successful geometric deep learning architecture such as the Graph Attention Network.\cite{velickovic2018graph} The networks, in addition to the typical metrics such as $r^2$, will be evaluated on their \textit{robustness} to missing or noisy data.
...
\begin{enumerate}[label=E\arabic*.]
  \item  Comparison of the alternative graph neural network models using $r^2$ metric.
...
\begin{itemize}
  ...
  \item Work on evaluation of the network: implement robustness measurement framework.
  \item Measure the robustness of the neural networks.
  \item Start writing the dissertation and project progress report.
\end{itemize}
...
\begin{itemize}
  ...
  \item Start on data preprocessing pipeline.
\end{itemize}
