...
where $\mathbf{X}$ and $\mathbf{y}$ are feature and label vectors, $\mathcal{L}(\cdot, \cdot)$ is the loss or error function used to train the model (in this project MSE), $f(\cdot)$ is the trained model, $\pi(\cdot)$ indicates a permutation function which uniformly returns a random permutation of the argument, and $k$ is the number of samples.
