...
The the purpose of the second type of robustness is to explore whether the population graph structure itself is useful in estimating the brain age for a particular node. To this end, instead of adding noise to the nodes, an increasing proportion of edges is removed from the population graph. The selected proportions of edges removed are the same as those in the previous section. For each of these levels the training procedure is carried out five times using a different random seed. The more edges are removed, the less neighbourhood structure the graph neural network models can exploit, having to rely on the node features only. 
