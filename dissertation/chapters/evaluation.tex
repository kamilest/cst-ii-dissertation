...
\label{section:discussion}
 The node noisification experiment shows that node corruption in the training set could
 the could drastically worsen the predictions for the uncorrupted test nodes, provided that enough nodes are corrupted. This result is expected because not only the noise could propagate to neighbourhoods, affecting individual predictions, but also because there is less useful training data available for the GNN architectures to learn from.
