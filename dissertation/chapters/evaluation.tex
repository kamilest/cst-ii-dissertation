...
For the first type of robustness evaluation, the normalised feature tensor \texttt{x} of the population graph (see Table~\ref{table:population-graph}) is perturbed with the additive white Gaussian noise of variance 0.5 for the increasing proportion of nodes – the levels chosen were 1\%, 5\%, 10\%, 20\%, 30\% and 50\%. Following the renormalisation of the tensor, the model is retrained again on the training dataset and tested on the hold-out test set, measuring the drop in performance. To make sure that any effect on the evaluation metrics is due to added noise and not the chaning dataset split, the model is trained on a single dataset split while the noise is added to different subjects. For each of the GCN and GAT models, this results in five training procedures at each noise level.
