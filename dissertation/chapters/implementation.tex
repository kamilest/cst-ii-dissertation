...
Since the original neuroimaging dataframes and brain health masks are kept unchanged in the data structure, the same population graph can be prepared for, say, a different training fold by simply going through the transformation component again but with a different training mask set, which will reset the values in the corresponding population graph fields. This explains the communication loop between the graph tranformation and GNN training and evaluation components in Figure~\ref{graph-transformation-component}.
