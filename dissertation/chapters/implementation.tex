...
The repository follows the structure of the five key project components as in Figure~\ref{pipeline-overview}. 

% \begin{table}[]
%     \caption{Summary of the non-imaging features used in this project.}\label{table:phenotype-features}
%     \centering
%     \begin{tabular}{lp{0.175\textwidth}p{0.475\textwidth}}
%         \hline
%     \textbf{Component} & \textbf{Filename} & \textbf{Description} \\ \hline
%     \multirow{2}{*}{XXX} & \texttt{ukb\_preprocess.py} & \\\cline{2-3}
%                      & \texttt{ukb\_preprocess_test.py} & \hline
%     \end{tabular}
% \end{table}
...
\begin{table}[]
    ...
    UKB preprocessing 
            & \texttt{phenotype.py} & Contains mappings of memorable non-imaging feature names to UKB codes, \\
    &        & \texttt{ukb\_preprocess.py} & Main preprocessing code (see Section~\ref{section:ukb-preprocessing})\\
    &        & \texttt{ukb\_preprocess\_test.py} &  Tests for similarity matrix code. \\ \hline
    \multirow{2}{*}{Graph construction} 
            & \texttt{graph\_construct.py} & \\
            & \texttt{graph\_construct\_test.py} &  \\ \hline
    Graph transformation
            & \texttt{graph\_transform.py} & \\
            % & \texttt{ukb\_preprocess\_test.py} &  \\   
    \multirow{2}{*}{GNN architectures} 
            & \texttt{ukb\_preprocess.py} & \\
            & \texttt{ukb\_preprocess\_test.py} &  \\   
    \multirow{2}{*}{GNN training} 
            & \texttt{ukb\_preprocess.py} & \\
            & \texttt{ukb\_preprocess\_test.py} &  \\ \hline
    \end{tabular}
    \end{table}