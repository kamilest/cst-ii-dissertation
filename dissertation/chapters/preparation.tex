...
Let $\mathbf{H}^{(k-1)} \in \mathbb{R}^{|V|\times C_{k-1}}$ be the input layer of a neural network with $C_{k-1}$ input channels, $\mathbf{H}^{(k)} \in \mathbb{R}^{|V|\times C_{k}}$ be the GCN layer with $C_{k}$ output channels, and $\mathbf{H}^{(0)} = \mathbf{X} \in \mathbb{R}^{|V| \times C_0}$ containing $C_0$ input features for each of the $|V|$ nodes in the population graph (in this case the neuroimaging data for $|V|$ subjects). For the weight matrix $\mathbf{\Theta}^{(k)} \in \mathbb{R}^{C_{k-1}\times C_{k}}$ and an activation function $\sigma(\cdot)$,

\begin{equation}
    \mathbf{H}^{(k)} \leftarrow \sigma( \mathbf{\tilde{D}}^{-1/2}\mathbf{\tilde{A}}\mathbf{\tilde{D}}^{-1/2}\mathbf{H}^{(k-1)}\mathbf{\Theta}^{(k)}).
\end{equation}
...
Analogously to the previous section, we consider an input layer $\mathbf{H}^{(k-1)} \in \mathbb{R}^{|V|\times C_{k-1}}$ (with $\mathbf{H}^{(0)} = \mathbf{X} \in \mathbb{R}^{|V|\times C_{0}}$ as before) and the GAT layer $\mathbf{H}^{(k)} \in \mathbb{R}^{|V|\times C_{k}}$. Write the input layer $\mathbf{H}^{(k-1)}$ as $[\mathbf{h}_1^{(k-1)} \cdots \mathbf{h}_{|V|}^{(k-1)}]^{\mathrm{T}}$, and $\mathbf{H}^{(k)}$ as $[\mathbf{h}_1^{(k)} \cdots \mathbf{h}_{|V|}^{(k)}]^{\mathrm{T}}$. For a trainable weight matrix $\mathbf{W}^{(k)} \in \mathbb{R}^{C_{k} \times C_{k-1}}$ and an \textit{attentional mechanism} $a: \mathbb{R}^{C_{k}} \times \mathbb{R}^{C_{k}} \rightarrow \mathbb{R}$ we compute (unnormalised) \textit{attentional coefficients} $e_{ij}$:

\begin{equation}
    e_{ij} = a(\mathbf{W}^{(k)}\mathbf{h}_i^{(k-1)}, \mathbf{W}^{(k)}\mathbf{h}_j^{(k-1)}).
\end{equation}

In Veli{\v{c}}kovi\'{c} et al.~\cite{velickovic2018graph}, the attention mechanism is implemented as a linear transformation using a learnable weight vector $\mathbf{a} \in \mathbb{R}^{2C_{k}}$, followed by a non-linearity $\sigma_1(\cdot)$):

\begin{align}
    e_{ij} &= a(\mathbf{W}^{(k)}\mathbf{h}_i^{(k-1)}, \mathbf{W}^{(k)}\mathbf{h}_j^{(k-1)}) \\
    &= \sigma_1(\mathbf{a}^{\mathrm{T}}[\mathbf{W}^{(k)}\mathbf{h}_i^{(k-1)} \parallel \mathbf{W}^{(k)}\mathbf{h}_j^{(k-1)}]),
\end{align}
...
\begin{align}
    ...
    &= \underset{{j: (i, j) \in E}}{\mathrm{softmax}}\left(\sigma_1(\mathbf{a}^{\mathrm{T}}[\mathbf{W}^{(k)}\mathbf{h}_i^{(k-1)} \parallel \mathbf{W}^{(k)}\mathbf{h}_j^{(k-1)}])\right) \\
    &=  \frac{\mathrm{exp}\left(\sigma_1(\mathbf{a}^{\mathrm{T}}[\mathbf{W}\mathbf{h}_i^{(k-1)} \parallel \mathbf{W}^{(k)}\mathbf{h}_j^{(k-1)}])\right)}{\sum\limits_{l: (i, l) \in E}\mathrm{exp}\left(\sigma_1(\mathbf{a}^{\mathrm{T}}[\mathbf{W}^{(k)}\mathbf{h}_i^{(k-1)} \parallel \mathbf{W}^{(k)}\mathbf{h}_l^{(k-1)}])\right)}.
\end{align}
...
\begin{equation}
    \label{eq:attention}
    \mathbf{h}_i^{(k)} = \sigma\left(\sum\limits_{j: (i, j) \in E} \alpha_{ij}\mathbf{W}^{(k)}\mathbf{h}_j^{(k-1)}\right)
\end{equation}
...
\begin{equation}
    \mathbf{h}_i^{(k)} = \underset{m=1}{\overset{M}{\big\|}} \sigma\left(\sum\limits_{j: (i,j)\in E} \alpha_{ij, m}\mathbf{W}^{(k)}_m\mathbf{h}_j^{(k-1)}\right).
\end{equation}
...
TODO notation: eqref instead of ref for equations, use $l_1,\dots, L$ for layers and $k$ for other things.
