...
Following the model proposed by Niu et al.~\cite{niu2019improved}, the brain age $y_b$ could be expressed as the sum of the known chronological age $y_c$ and the unknown \textit{brain age gap} $\varepsilon_g$ that defined as the discrepancy between the chronological and the brain age:
...
The features derived from structural MRI that will be used for this project are \textit{cortical thickness}, \textit{surface area} and \textit{grey matter volume} (see Table~\ref{table:structural-features}), reported at parcel granularity.
...
The two types of an fMRI image is \textit{task fMRI}, when the subject is asked to perform a specific cognitive task, and \textit{resting state fMRI} (rs-fMRI), when the subject is not performing any particular task. This project will use rs-fMRI data.
