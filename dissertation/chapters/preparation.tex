...
\begin{gather}
    T_i(\mathbf{x}) = 2\mathbf{x}T_{i-1}(\mathbf{x}) - T_{i-2}(\mathbf{x}) \\
    T_1 = \mathbf{x} \\
    T_0 = 1
\end{gather}
...
\begin{equation}
    \mathbf{\hat{G}} \approx \sum_{i = 0}^{k} \theta_i T_i(\mathbf{\tilde{\Lambda}})
\end{equation}

where $\mathbf{\tilde{\Lambda}} = 2\mathbf{\Lambda}/\lambda_{\mathrm{max}} - \mathbf{I}$, $\lambda_\mathrm{max}$ is the largest eigenvalue in $\mathbf{\Lambda}$, and $\theta_i$ are coefficients such that $\mathbf{\hat{G}} = \sum_i \theta_i \mathbf{\Lambda}^i$~\cite{wu2019simplifying}. The truncation coefficient (the degree of the polynomial) $k$ corresponds to the neighbourhood of at most $k$ hops away from the node of interest. 

Next, it can be proved~\cite{wu2019comprehensive} that

\begin{equation}
    T_i(\mathbf{\tilde{L}}) = \mathbf{U}T_i(\mathbf{\tilde{\Lambda}})\mathbf{U}^{\mathrm{T}}
\end{equation}

with $\mathbf{\tilde{L}} = 2\mathbf{L}/\lambda_{\mathrm{max}} - \mathbf{I}$. The result is the approximated spectral graph convolution:

\begin{equation}
    \mathbf{x} *_G \mathbf{g} \approx \sum_{i=0}^{k}\theta_i T_i(\mathbf{\tilde{L}})\mathbf{x}.
\end{equation}
...
\subsubsection{Kipf and Welling's graph convolutional networks}
assume $k=1$, $\lambda_{\mathrm{max}}=2$ so that the polynomial in 2.18 simplifies to only two learnable parameters but even they can be further simplified to 1 parameter.
...
% extension: an alternative to Chebyshev polynomials (extension): cayleynet--probably too difficult

\subsection{Layers of the graph convolutional network}
