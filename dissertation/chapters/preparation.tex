...
From Equations~\eqref{brainage:eq1} and~\eqref{brainage:eq2}, the estimate of chronological age is, on the other hand,
...
TODO assign $|V| = N$ which would look nicer throughout the sections.
...
The graph convolution operation in Equation~(\ref{eq:gcnconv2}) is generalised to the entire layer of the network as follows~\cite{wu2019simplifying}. 

Let $\mathbf{H}^{(l-1)} \in \mathbb{R}^{|V|\times C_{l-1}}$ be the input layer of a neural network with $C_{l-1}$ input channels, $\mathbf{H}^{(l)} \in \mathbb{R}^{|V|\times C_{l}}$ be the GCN layer with $C_{l}$ output channels, and $\mathbf{H}^{(0)} = \mathbf{X} \in \mathbb{R}^{|V| \times C_0}$ containing $C_0$ input features for each of the $|V|$ nodes in the population graph (in this case the neuroimaging data for $|V|$ subjects). For the weight matrix $\mathbf{\Theta}^{(l)} \in \mathbb{R}^{C_{l-1}\times C_{l}}$ and an activation function $\sigma(\cdot)$,
...
In contrast to the \textit{spectral} GCN that relies on the computation within the Fourier domain using the graph Laplacian, the graph attention network (GAT)~\cite{velickovic2018graph} is a \textit{spatial} architecture that operates in the Euclidean domain. Here the mechanism that incorporates the neighbourhood information is defined to work on the neighbourhoods of different sizes, with each node learning the how much importance (\textit{attention}) it should assign to each one of its neighbours.
...
Analogously to the previous section, we consider an input layer $\mathbf{H}^{(l-1)} \in \mathbb{R}^{|V|\times C_{l-1}}$ (with $\mathbf{H}^{(0)} = \mathbf{X} \in \mathbb{R}^{|V|\times C_{0}}$ as before) and the GAT layer $\mathbf{H}^{(l)} \in \mathbb{R}^{|V|\times C_{l}}$. Write the input layer $\mathbf{H}^{(l-1)}$ as $[\mathbf{h}_1^{(l-1)} \cdots \mathbf{h}_{|V|}^{(l-1)}]^{\mathrm{T}}$, and $\mathbf{H}^{(l)}$ as $[\mathbf{h}_1^{(l)} \cdots \mathbf{h}_{|V|}^{(l)}]^{\mathrm{T}}$. For a trainable weight matrix $\mathbf{W}^{(l)} \in \mathbb{R}^{C_{l} \times C_{l-1}}$ and an \textit{attentional mechanism} $a: \mathbb{R}^{C_{l}} \times \mathbb{R}^{C_{l}} \rightarrow \mathbb{R}$ we compute (unnormalised) \textit{attentional coefficients} $e_{ij}$:
...
\begin{enumerate}
    \item Implementation of the population graph data structure, with nodes containing the individual neuroimaging data and edges representing associations between them based on pairwise similarity. This involves: \begin{enumerate}
        \item Preprocessing of the functional, structural imaging and phenotypic (non-imaging) data of the UK Biobank subjects.
    ...
    \item Training and evaluation of the two graph neural network architectures on the UK Biobank population graph. This includes the \textit{extension} of implementing the robustness measurement framework.
\end{enumerate}
...
To track the workflow, prevent loss of data and ease the readability and/or any future extensions to this project, the common practices of code style, documentation, unit testing, version control and code reuse will be extensively applied.

\section{Choice of tools}

This project will use \textit{PyTorch Geometric}~\cite{fey2019pytorch}, an extension library for \textit{PyTorch}~\cite{pytorch} that implements a variety of graph neural network architectures, including GCNs and GATs. The library is only supported in Python, which will be the implementation language of this project. For scientific computing I will use common Python packa such as \textit{scikit-learn}~\cite{pedregosa2011scikit}, \textit{pandas}~\cite{mckinney2010scipy}, and \textit{NumPy}~\cite{walt2011numpy}. 

The neuroimaging machine learning library \textit{Nilearn}\footnote{https://nilearn.github.io} will be used for some of the neuroimaging data processing and visualisation~\cite{abraham2014machine}.

For model logging, visualisation and hyperparameter tuning, I will use the free \textit{Weights \& Biases}~\cite{wandb} framework.
...
\begin{itemize}
    ...
    \item previous Part IB courses like Artificial intelligence on the knowledge of which a lot of the preparation section material is based on.
\end{itemize}