...
To avoid the $O(n^2)$ filtering operation due to the matrix-vector multiplications in Equation~(\ref{eq:convolution}), Defferrard et al.'s~\cite{defferrard2016convolutional} ChebNets use recursively defined \textit{Chebyshev polynomials}
...
\begin{enumerate}
    \item Taking $K=1$ makes the Equation~(\ref{eq:chebnet}) linear, which is more suitable for neural network architectures with linear layers and non-linearities between them.
