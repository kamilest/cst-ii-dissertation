...
input node features $\mathbf{h} = \{\vec{h}_1, \dots, \vec{h}_N\}, \vec{h_i} \in \mathbb{R}^F$

output node features $\mathbf{h}' = \{\vec{h}_1', \dots, \vec{h}_N'\}, \vec{h}_i' \in \mathbb{R}^{F'}$
...
attention coefficients \textit{can already include neighbourhoods} $e_{ij} = \mathbf{a}(\mathbf{W}\vec{h}_i, \mathbf{W}\vec{h}_j)$
...
\begin{align}
    \alpha_{ij} &= \mathrm{softmax}_j(\mathbf{a}^T[\mathbf{W}\vec{h}_i \parallel \mathbf{W}\vec{h}_j]) \\
    &=  \frac{\mathrm{exp}\left(\sigma_1(\mathbf{a}^T[\mathbf{W}\vec{h}_i \parallel \mathbf{W}\vec{h}_j])\right)}{\sum\limits_{k \in \mathcal{N}_i}\mathrm{exp}\left(\sigma_1(\mathbf{a}^T[\mathbf{W}\vec{h}_i \parallel \mathbf{W}\vec{h}_k])\right)}
\end{align}

where $\alpha_{ij}$ is the attention coefficient for an edge $i\leadsto j$ (corresponding to the importance of features in node $j$ to the features in node $i$), normalised across $i$'s neighbourhood $\mathcal{N}_i$ (defined as e.g. all nodes one hop away); $\sigma_1$ is a non-linearity.

The coefficients $\alpha_{ij}$ and the weight matrix are used to compute the output features:

\begin{equation}
    \mathbf{h}_i = \sigma(\sum\limits_{k \in \mathcal{N}_i} \alpha_{ij}\mathbf{W}\mathbf{h}_j)
\end{equation}
