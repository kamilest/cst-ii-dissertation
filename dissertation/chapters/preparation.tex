...
The graph convolution operation in~(\ref{eq:gcnconv2}) is generalised to apply to an entire layer of the network as follows. 

Let $\mathbf{H}^{(k-1)} \in \mathbb{R}^{|V|\times C_{k-1}}$ be the input layer of a neural network with $C_{k-1}$ input channels, $\mathbf{H}^{(k)} \in \mathbb{R}^{|V|\times C_{k}}$ be the output layer with $C_{k}$ output channels, and $\mathbf{H}^{(0)} = \mathbf{X} \in \mathbb{R}^{|V| \times C_0}$ containing $C_0$ input features for each of the $|V|$ nodes in the population graph (in this case the neuroimaging data for $|V|$ subjects). For the weight matrix $\mathbf{\Theta}^{(k)} \in \mathbb{R}^{C_{k-1}\times C_{k}}$ and an activation function $\sigma(\cdot)$,
