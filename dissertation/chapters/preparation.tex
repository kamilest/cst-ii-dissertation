...
Following the model proposed by Niu et al.~\cite{niu2019improved}, the brain age $y_b$ could be expressed as the sum of the known chronological age $y_c$ and the unknown \textit{brain age gap} $\varepsilon_g$, defined as the discrepancy between the chronological and the brain age:
...
It is generally assumed~\cite{franke2019ten} that, for healthy subjects in the general population, the brain age corresponds to chronological age:
...
Since the true brain age is unknown, any (semi-)supervised machine learning model can only use chronological age as labels, following Equation~\eqref{brainage:eq3}.

% When the model is trained only on healthy subjects, $f(\cdot)$ will tell how much of \textit{chronological} age can be explained by $X$, but because for healthy subjects $\varepsilon \approx \varepsilon_e$ (which follows from Equation~\eqref{brainage:assume}). When applied to non-healthy subject, the same $f(\cdot)$ will explain the chronological age, leaving the

% When the model is trained only on healthy subjects, $f(\cdot)$ will tell how much of \textit{chronological} age can be explained by $X$. When the same model is applied to non-healthy subject, 

% However, the assumption in Equation~\eqref{brainage:assume} still permits learning the brain age function $f(\cdot)$ as long as the model is trained only on healthy subjects, so that $\varepsilon \approx \varepsilon_e$. When the same model is applied to subjects with various brain health conditions, the errors outside of the $\varepsilon_e$ could be assumed to come from the brain age gap. 
